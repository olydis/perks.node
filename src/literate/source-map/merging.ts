  ...
  if (value) {
    ...
    if (value instanceof Array) {
      ...
      for (const each of value) {
        ...
        result.push(<never>resolveRValue(each, "", higherPriority || lowerPriority, null));
      }
      return result;
    }
  }
...
export function MergeYamls<T>(config: ConfigurationView<T>, yamlInputHandles: DataHandleRead[], yamlOutputHandle: DataHandleWrite): Promise<DataHandleRead> {
