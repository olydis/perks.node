...
import { OperationAbortedException } from "@microsoft.azure/polyfill";
...
export async function Parse<T>(config: ConfigurationView<T>, literate: DataHandleRead, workingScope: DataStoreView): Promise<DataHandleRead> {
...
export async function ParseCodeBlocks<T>(config: ConfigurationView<T>, literate: DataHandleRead, workingScope: DataStoreView): Promise<Array<CodeBlock>> {
  const docScope = workingScope.CreateScope(`doc_tmp`);
  const hwRawDoc = await workingScope.Write(`doc.yaml`);
  return await ParseCodeBlocksInternal(config, literate, hwRawDoc, docScope);
}

async function ParseInternal<T>(config: ConfigurationView<T>, hLiterate: DataHandleRead, hResult: DataHandleWrite, intermediateScope: DataStoreView): Promise<DataHandleRead> {
...
async function ParseCodeBlocksInternal<T>(config: ConfigurationView<T>, hLiterate: DataHandleRead, hResult: DataHandleWrite, intermediateScope: DataStoreView): Promise<CodeBlock[]> {
