...
export class ExtensionManager {
  ...
  public async start(extension: Extension): Promise<childProcess.ChildProcess> {
    ...
    if (command[0] == 'node') {
      ...
      return childProcess.fork(command[1], command.slice(2), { cwd: extension.modulePath, env: env, silent: true })
    }
    // spawn the command 
    return childProcess.spawn(command[0], command.slice(1), { cwd: extension.modulePath, env: env });
  }
}